...
function program_connect(port) {

    var prg_sockjs_url = 'http://localhost:' + port + '/test';		 
    var prg_sockjs = new SockJS(prg_sockjs_url);

    prg_sockjs.onopen    = function(){
         $('#consoleOutput').append("PROGRAM SOCKJS OPENED" + "<br>");
    };
    
    prg_sockjs.onmessage = function(e){
            var pydat = JSON.parse(e.data);
            $('#consoleOutput').append(pydat["name"]);
    };
    
    prg_sockjs.onclose   = function(){
        $('#consoleOutput').append("PROGRAM SOCKJS CLOSED");
    };
